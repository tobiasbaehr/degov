...
class RoboFile extends \Robo\Tasks
{
  ...
  protected function runComposerUpdate(): void {
    ...
    $this->taskExecStack()
      ...
      ->exec('cd ' . self::ROOT_FOLDER . '&& composer update nrwgov/nrwgov --with-dependencies')
      ->run();
  ...
  protected function runTranslationsUpdate(): void {
    ...
    $this->taskExecStack()
      ->setVerbosityThreshold(VerbosityThresholdInterface::VERBOSITY_DEBUG)
      ->exec('cd ' . self::ROOT_FOLDER . '&& drush locale-check')
      ->run();
    ...
    $this->taskExecStack()
      ->setVerbosityThreshold(VerbosityThresholdInterface::VERBOSITY_DEBUG)
      ->exec('cd ' . self::ROOT_FOLDER . '&& drush locale-update')
      ->run();
  ...
  protected function runDrupalUpdateHooks(): void {
    ...
    $this->taskExecStack()
      ->setVerbosityThreshold(VerbosityThresholdInterface::VERBOSITY_DEBUG)
      ->exec('cd ' . self::ROOT_FOLDER . '&& drush updb')
      ->run();
  ...
  protected function runEntityUpdates(): void {
    ...
    $this->taskExecStack()
      ->setVerbosityThreshold(VerbosityThresholdInterface::VERBOSITY_DEBUG)
      ->exec('cd ' . self::ROOT_FOLDER . '&& drush entup')
      ->run();
  ...
  protected function runConfigurationExportIntoFilesystem(): void {
    ...
    $this->taskExecStack()
      ->setVerbosityThreshold(VerbosityThresholdInterface::VERBOSITY_DEBUG)
      ->exec('cd ' . self::ROOT_FOLDER . '&& drush cex')
      ->run();
  ...
  protected function runBaseThemeNpmPackageUpdate(): void {
    ...
    $this->_exec('cd ' . self::ROOT_FOLDER . '&& cd docroot/themes/nrw/nrw_base_theme && nvm install 6.11.3 && npm i');
  ...
  protected function runCustomThemeUpdate(): void {
    ...
    if ($themeConfig['default'] !== 'nrw_base_theme') {
      $this->_exec('cd ' . self::ROOT_FOLDER . '&& cd docroot/themes/custom/' . $themeConfig['default'] . ' && nvm install 6.11.3 && npm i');
    }
