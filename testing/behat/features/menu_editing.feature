...
#    And I should see HTML content matching "word-document-2.docx"
  ...
  Scenario: I check that menu item autocomplete suggestions include Media entities
    ...
    And I am logged in as a user with the "administrator" role
    And I am on "/admin/structure/menu/manage/main/add"
    And I should not see HTML content matching "A document with a fixed title [Dokument, word-document.docx]"
    ...
    Then I should see HTML content matching "A document with a fixed title [Dokument, word-document.docx]" after a while
