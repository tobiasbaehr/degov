...
Feature: deGov - Paragraphs
  ...
  Scenario: Paragraph bock reference has correct blocks and can create an instance
    Given I am logged in as a user with the "administrator" role
    And I am on "/node/add/normal_page#edit-group-sidebar"
    And I fill in "testblockreferenz" for "Titel"
    And I click on togglebutton
    And I press "edit-field-sidebar-paragraphs-add-more-add-more-button-block-reference-sidebar"
    And I should see text matching "Block Referenz" after a while
    Given Select "field_sidebar_paragraphs[0][subform][field_block_plugin][0][plugin_id]" has following options "views_block:press_latest_sidebar-latest_press simplenews_subscription_block"
    And I select "simplenews_subscription_block" from "field_sidebar_paragraphs[0][subform][field_block_plugin][0][plugin_id]"
